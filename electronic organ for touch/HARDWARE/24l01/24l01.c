...
void NRF24L01_RX_Mode()
{
  	...
  	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(u8*)ADDRESS_MAIN,TX_ADR_WIDTH);//写RX节点地址
	  
  	NRF24L01_Write_Reg(NRF_WRITE_REG+EN_AA,0x03);    //使能通道0的自动应答    
  	NRF24L01_Write_Reg(NRF_WRITE_REG+EN_RXADDR,0x03);//使能通道0的接收地址  
  	...
  	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,SI24R1_REC_CHA);	     //设置RF通信频率
	
  	NRF24L01_Write_Reg(NRF_WRITE_REG+RX_PW_P0,TX_PLOAD_WIDTH);//选择通道0的有效数据宽度 	    
	 
		NRF24L01_Write_Reg(NRF_WRITE_REG+RF_SETUP,RF_SETUP_SET);//设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
...
{														 
  ...
  //	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(u8*)RX_ADDRESS,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  
  	...
  	NRF24L01_Write_Reg(NRF_WRITE_REG + SETUP_RETR, 0x1a);     //使能通道(全部)的自动应答    
  	NRF24L01_Write_Reg(NRF_WRITE_REG + RF_CH, SI24R1_SEND_CHA); //  
  	NRF24L01_Write_Reg(NRF_WRITE_REG + RF_SETUP, RF_SETUP_SET);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
...
u8 NRF24L01_TxPacket(u8 *txbuf,  u8 sendAddr)
{
	...
	SPI2_SetSpeed(SPI_BaudRatePrescaler_8);//spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   
	NRF24L01_Write_Buf(NRF_WRITE_REG+TX_ADDR,(u8*)ADDRESS_SLAVE_ONE,TX_ADR_WIDTH);//写TX节点地址 
	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(u8*)ADDRESS_SLAVE_ONE,TX_ADR_WIDTH);//写TX节点地址
